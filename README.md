...
| `POST /order/api/account/phone` `POST /order/api/customer` | Sunshine phone login / signup. Returns `session_token` (no email). |
| `POST /order/api/account/profile` `POST /order/api/customer/profile` | Update Square name / email for the session customer. Requires session token. |
| `GET /order/api/account` `GET /order/api/customer` `GET /order/api/account/status` `GET /order/api/orders` | Account + orders. Requires `Authorization: Bearer <session_token>` (or `X-Session-Token`). |
...
`?minutes=15` is the default window. Newest tickets sit first. Tap a ticket when the drink is done and it drops off. The list refreshes every 10 seconds. Only Square **Drink** category lines belong on the board (category ids `BYKQS3P2SI7WP22F6BWKFZGR`, `OCV6MHUVAXUXXXFATFKLJNPI`, `ROPXOXPWBYM42T3LJQESG3NX`, or exact category name `Drink` — not Drink Station). A name needle of `biscoff` is not used; that was putting Biscoff Roll on the board. Tap-to-clear appends the order to a client-side pushed list (`kds_cleared` in localStorage and sessionStorage). If an order is in that list, do not show it — on tap (~1s), when the next `#ticket-list` poll arrives (strip cleared `order_id`s from the response HTML before HTMX inserts it), after swap (~12s, belt-and-suspenders hide), and after refresh. Server `cleared=` / cookie are optional backup only.
...
`BAKERY_SERVICE=drinks` 404s `/loyalty` (real phones). Public drinks fetches `GETORDERS_URL` on every refresh, groups by order, and tap-to-clear hides that order on the tablet (cookie, not a ticket DB). Irondale QR order is `/order` on this same service: phone menu from **Square Catalog** (Drink category at Irondale, full POS modifier lists) → Square CreatePaymentLink with catalog variation/modifier ids (token stays in Secret Manager) → status page. Paid drinks still land on the board the same way as POS (Square → getorders). See `DEPLOY.md` for env vars, token scopes (**ITEMS_READ** plus checkout write scopes), and how to print the tent QR.

Sunshine Android phone login is POST-only on this same drinks service (`POST /order/api/account/phone` or `POST /order/api/customer`). The response includes a short-lived `session_token` (HMAC, 30 days). The APK must store that token and send it on later GETs (`Authorization: Bearer <session_token>` or `X-Session-Token`). GET without a token is 401 — phone / `customer_id` query params are not enough. When the Square customer is missing a name, the Godot form POSTs `/order/api/account/profile` (alias `/order/api/customer/profile`) with `{given_name, family_name, email}` (all optional, at least one required) and the same session token. The drinks service updates that session's Square customer only — do not send a client `customer_id` as the update target. Profile response matches login (`ok`, `customer` without email, `loyalty`, `orders`, refreshed `session_token`). Account JSON never includes email. `SESSION_SECRET` is optional (see `DEPLOY.md`); `SQUARE_ACCESS_TOKEN` stays server-side only.

New drink lines ding once on the drinks board (not first load, not tap-off, not an unchanged 10s poll). First tap is a full-screen overlay that plays a test ding (iPad Safari unlock; later polls reuse that audio). Cleared orders stay off that iPad via localStorage. Laptop ingest is unchanged when `GETORDERS_URL` is unset. `INGEST_KEY` and `SQUARE_ACCESS_TOKEN` come from Secret Manager, not git. Header `X-Ingest-Key`. Sqlite on Cloud Run min 0 is ephemeral; laptop still sqlite.
