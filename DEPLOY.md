...
5. After pay, Square redirects to `/order/status`. Labels **Paid** and **Making** (not color-only). Those screens list the ordered drinks with an in-progress spinner — not a single drink photo. Copy is “We're making it” / “We'll text when it's at pickup” if a phone was given **and** Twilio is configured. Otherwise “We'll have it at pickup.”
